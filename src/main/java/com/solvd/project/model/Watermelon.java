...
public class Watermelon extends Melon implements Product, Harvestable, Storable, Pricable {
    ...
    private static final Logger logger = LogManager.getLogger(Watermelon.class);

    public Watermelon(double weight, boolean ripe, LocalDate harvestDate, int daysToSpoil, boolean organic,
            RipenessLevel ripenessLevel, StorageType storageType, ProcessingStage stage) {
        this.weight = weight;
        this.ripe = ripe;
    ...
    @Override
    public double getTotalPrice() throws PriceException {
        ...
        if (daysRemaining <= 2) {
            basePrice *= 0.85;
        } else if (pricePerKg > basePrice) {
            throw new PriceException("The product is too expensive.", new Exception());
        }
    ...
    public void harvest() {
        if (ripe) {
            System.out.println("Harvesting big watermelon 🍉... requires two hands!");
        } else {
    ...
    public void chill() {
        logger.info("Chilling the watermelon 🍉...");
    }
}
